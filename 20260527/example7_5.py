...
## a.sql 語法
completed = '''
SELECT order_id,customer_id,payment_type,status
FROM orders
...
'''
with sql.connect('data/raw/course.db') as conn:
    df= pd.read_sql_query(completed,conn)
    print(df)
print("="*30)

## b.利用 dataframe loc 或 query 方法查詢篩選
df= orders.loc[(orders['status']=='completed')&(orders['payment_type']=='card'),['order_id','customer_id','payment_type','status']].head()
print(df)
print("="*30)
print('利用 query 方法')
result=orders.query(" status=='completed' and payment_type == 'card'")[['order_id','customer_id','payment_type','status']].head()
print(result)