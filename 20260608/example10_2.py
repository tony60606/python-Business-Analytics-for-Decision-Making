...
print(facts['line_revenue'].describe().round(2))
print("="*30)
print(facts[['line_revenue']].describe().round(2))
print("="*30)
#印出分組-客群來源的平均營收,遞減排序
print(facts.groupby('acquisition_channel')['line_revenue'].mean().round(2).sort_values(ascending=False))
print("="*30)
...
ax.set_title('各付款方式訂單數',fontsize=20)

#設定 x 軸標籤為 付款方式
ax.set_xlabel('付款方式',fontsize=16)

#設定 y 軸標籤為 訂單數
ax.set_ylabel('訂單數',fontsize=16)
