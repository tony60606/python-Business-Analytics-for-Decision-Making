...
orders_with_customers=orders.merge(customers[['customer_id','segment','city','acquisition_channel']],on='customer_id',how='left')
...
print(f'兩資料長度是否一致：{len(orders_with_customers) == len(orders)}\n'
      f'orders_with_customers紀錄筆數：{len(orders_with_customers)}筆\n'
      f'orders紀錄筆數：{len(orders)}筆')
...
items_enriched=(
    order_items.merge(orders[['order_id','customer_id','order_date','status','payment_type']],on='order_id',how='inner').merge(products[['product_id','category']],on='product_id',how='left')
)
## 計算每個品項的營收
items_enriched["line_revenue"] = (
    items_enriched["quantity"] * items_enriched["unit_price"] * (1 - items_enriched["discount_rate"])
    ).round(2)

items_enriched[['order_id','product_id','category','quantity','unit_price','discount_rate','line_revenue']].to_csv('temp3.csv')
