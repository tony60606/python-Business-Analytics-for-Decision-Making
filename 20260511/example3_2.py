...
refunded2 = orders.loc[(orders['order_date'].dt.year == 2025) & (orders['order_date'].dt.month <= 6) & (orders['status'] == 'refunded'),['order_id','order_date','status']]
refunded2.to_csv('refunded2.csv')
print(f'2025上半年度退款訂單=\n{refunded}')
print(f'2025年上半年退款訂單比數={len(refunded2):,}筆')