...
-- 2. 偏移函數 (依部門分組,按月份由小到大排序)
    LAG(Sales,1) OVER (PARTITION BY Dept ORDER BY Month) AS sql_lag,
    LEAD(Sales,1) OVER (PARTITION BY Dept ORDER BY Month) AS sql_lead,
  

-- 3. 聚合函式 ()
    SUM(Sales) OVER (PARTITION BY Dept ORDER BY Month) As sql_sum ,
    AVG(Sales) OVER (PARTITION BY Dept ORDER BY Month ROWS BETWEEN 1 PRECEDING AND CURRENT ROW) As sql_avg_2m
 FROM salestable
 ORDER BY Dept,Month;  
