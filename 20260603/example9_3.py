...
query3='''
SELECT id,date,revenue,
SUM(revenue) OVER (ORDER BY date ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING) AS sum2
FROM sales
'''
print("="*30)
...
query4='''
SELECT id,date,revenue,
SUM(revenue) OVER (ORDER BY date ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS sum2 ,
SUM(revenue) OVER () AS sum3
FROM sales
'''
print("="*30)
